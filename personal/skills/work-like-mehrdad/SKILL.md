...
- **Craft baseline.** Precise domain-correct names — rename bad ones on sight. Named constants over magic literals; every usage replaced. Configurable over hardcoded env values. Stale comments and dead conditionals are defects.
- **Communicate in STE.** Short sentences, active voice, one idea per sentence, always the why. Show code or an example instead of prose. Writeups state what/why, never process; write for the actual audience.
